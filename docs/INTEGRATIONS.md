...
Connect from BioTwin. Scopes are `calendar.readonly`, `calendar.events`, and `tasks.readonly`. Enable the Google Tasks API too if Tasks should appear. Existing connections can reconnect to grant the additional Tasks scope; declining it does not hide calendar events. The primary calendar timezone is read after consent, and the app opens Daily plan. The agenda fetches every provider page across all readable Google calendars for the selected day, with recurring instances expanded. FreeBusy remains authoritative for conflicts.

Daily plan shows one day at a time: named events, all-day entries, details/links and tasks due that day, with previous/Today/next controls, a date picker, search and calendar/task filters. It refreshes after connecting/adding, on window focus, and every minute while visible. Moving to another date requests that exact local day; there is no first-page event cutoff.
