...
- Added shared `FitnessHarness` contracts:
  - `HarnessMetric`
  - `HarnessDecision`
  - `FitnessHarness`
...
1. Make the planner directly consume `FitnessHarness.policy` instead of recomputing similar rules inside `planning.py`.
