...
const stats=await page.evaluate(()=>new Promise(resolve=>{
  ...
  function tick(now){if(now-start>2500)samples.push(now-previous);previous=now;if(now-start<62500){requestAnimationFrame(tick);return;}samples.sort((a,b)=>a-b);resolve({frames:samples.length,p50:samples[Math.floor(samples.length*.5)],p95:samples[Math.floor(samples.length*.95)],p99:samples[Math.floor(samples.length*.99)],max:samples.at(-1)});}
  requestAnimationFrame(tick);
}));
await page.screenshot({path:'test-results/exercise-gesture.png',fullPage:true});
await writeFile('test-results/performance.json',JSON.stringify(stats,null,2));
