...
try {
  ...
  await page
    ...
    .fill(`test-${Date.now()}@biotwin.invalid`);
  await page
    .getByLabel("Password", { exact: true })
    .fill("biotwin-local-test-password");
  ...
  const [chooser] = await Promise.all([
    ...
    page
      .getByRole("button", { name: "Import Garmin data", exact: true })
      .click(),
  ]);
  await chooser.setFiles("fixtures/golden/generated-recovery.fit");
  await page
    .getByText("Imported 37 measurements · 0 duplicates skipped.", {
      exact: true,
    })
    .waitFor();
  await page
    ...
    .click();
  await page
    .getByRole("heading", { name: "Heart rate", exact: true })
    .waitFor();
  await page.screenshot({
    path: "test-results/imported-signals.png",
    fullPage: true,
  });
  await page
    .getByRole("navigation", { name: "Main navigation", exact: true })
  ...
  console.log(
    "Browser account creation, original FIT import, signals rendering and account deletion passed.",
  );
} finally {
  await browser.close();
}
