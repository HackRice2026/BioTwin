#!/usr/bin/env python3
"""Build a leakage-aware 5-minute Garmin training table for MATLAB.
...
Output (default):
    processed_data/garmin_5min_master.csv
...
def resample_sensor(df: pd.DataFrame, prefix: str):
    """5-minute sensor statistics, including slope and sample count."""
    if df.empty:
        return pd.DataFrame()
...
def add_body_battery_labels(master: pd.DataFrame, anchors: pd.DataFrame):
    ...
    half_window_ns = int((WINDOW_MINUTES / 2) * 60 * 1e9)
    ...
    for i, t in enumerate(mt):
        ...
        # A real Garmin anchor within +/- half the window represents this 5-min bin.
        if candidates:
            nearest = min(candidates, key=lambda j: abs(at[j] - t))
            if abs(at[nearest] - t) <= half_window_ns:
                values[i] = av[nearest]
                weights[i] = 1.0
                nearby[i] = 1
                gaps[i] = 0.0
...
def add_rolling_features(df: pd.DataFrame):
    ...
    for col in ["hr_mean", "stress_mean", "resp_mean"]:
        if col in out.columns:
            out[f"{col}_1h"] = out[col].rolling(12, min_periods=3).mean()
            out[f"{col}_3h"] = out[col].rolling(36, min_periods=9).mean()

    if "bb_current_last_known" in out.columns:
        # Causal Body Battery trend features based only on values already observed.
        out["bb_current_change_1h"] = out["bb_current_last_known"].diff(12)
        out["bb_current_change_3h"] = out["bb_current_last_known"].diff(36)

    if "body_battery" in out.columns:
        # Future targets. These are LABELS only; never include them as predictors.
        for name, steps in [("30m", 6), ("1h", 12), ("3h", 36), ("6h", 72)]:
...
def main():
    ...
    parser.add_argument("--output", default="processed_data/garmin_5min_master.csv")
    ...
    master["sensor_window_complete"] = (
        (master.get("hr_count", 0).fillna(0) >= 2)
        & (master.get("stress_count", 0).fillna(0) >= 1)
        & (master.get("resp_count", 0).fillna(0) >= 2)
    ).astype(int)
    ...
    print(f"5-min bins near a real Garmin BB anchor: {anchors:,}")
